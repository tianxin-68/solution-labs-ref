package com.bi.queryer.util.annotation;
...
public class AnnotationScanner {
    ...
    public static Set<Class<?>> scanClassesByAnnotations(String pkgName, final boolean recursive, Class<? extends Annotation> targetAnnotation){
    	Map<Class<? extends Annotation>, Set<Class<?>>> classMap = scanClassesByAnnotations(pkgName, recursive, Arrays.asList(targetAnnotation));
    	if(classMap == null || classMap.isEmpty()) {
    		return null;
    	}
    	return classMap.get(targetAnnotation);
    }
    ...
    private static Class<?> getClassObj(File file, String pkgPath, String pkgName) throws ClassNotFoundException{
        // 考虑class文件在子目录中的情况
        String absPath = file.getAbsolutePath().substring(0, file.getAbsolutePath().length() - EXT.length() - 1).replaceAll("\\\\", "/");
        String className = absPath.substring(absPath.lastIndexOf("/") + 1) ;//absPath.substring(pkgPath.length()).replace('/', '.');
