...
/**
 * @Author contributor
 * @Date 17:18 2023-06-30
 * @Description
 * 1、单个模型的数据源sql构建器：处理自定义维度、明细数据过滤，不做聚合
 * 2、额外添加同环比对比所需日期
 **/
public class AnalysisDataSourceSqlBuilder extends DefaultDataSourceSqlBuilder {
    ...
    public AnalysisDataSourceSqlBuilder(StarModel model, QueryConfigure config, QueryContext cxt, AnalysisDataSourceCfg cfg) {
    ...
    public List<String> buildExtendWhereFragments(List<QueryField> filterFields) {
        ...
        List<String> whereFragments = super.buildExtendWhereFragments(filterFields);
        //非日粒度不处理
        DateGranularity dateGranularity = DateGranularity.get(this.config.getSettings().getDateGranularity());
        if (DateGranularity.DAY != dateGranularity) {
            return whereFragments;
        }

        if (AnalysisCalcMode.TB_MONTH == analysisCalcMode || AnalysisCalcMode.TB_YEAR == analysisCalcMode || AnalysisCalcMode.TB_YEAR_2 == analysisCalcMode || AnalysisCalcMode.TB_YEAR_3 == analysisCalcMode) {
            QueryField commonDateField = filterFields.stream().filter(QueryField::isCommonDate).findFirst().orElse(null);
            if (commonDateField == null) {
                return whereFragments;
            }
            List<String> excludeDateList = AnalysisUtil.getExcludeTbDateList(config, analysisCalcMode, compareDateRange);
            if (CollUtil.isNotEmpty(excludeDateList)) {
                whereFragments.add(String.format(" %s not in (%s) ",
                        getWhereFieldFullName(commonDateField),
                        BIUtil.listToStr(excludeDateList, ",")));
            }
        }
